...
class UserSignup extends Database
{
    ...
    public function update($data, $id_player)
    {
        ...
        $sql = "UPDATE {$this->tableName} SET {$fileds} WHERE id_user=:id_user";
        $stmt = $this->conn->prepare($sql);
        try {
            $this->conn->beginTransaction();
            $data['id_user'] = $id_player;
            $stmt->execute($data);
            $this->conn->commit();
        } catch (PDOException $e) {
            echo "Error: " . $e->getMessage();
    ...
    {
        $sql = "SELECT * FROM {$this->tableName} ORDER BY id_user DESC LIMIT {$start},{$limit}";
        $stmt = $this->conn->prepare($sql);
        $stmt->execute();
        if ($stmt->rowCount() > 0) {
    ...
    {
        $sql = "DELETE FROM {$this->tableName}  WHERE id_user=:id_user";
        $stmt = $this->conn->prepare($sql);
        try {
    ...
    {
        $sql = "SELECT * FROM {$this->tableName} WHERE username LIKE :search ORDER BY id_user DESC LIMIT {$start},{$limit}";
        $stmt = $this->conn->prepare($sql);
